...
# Шпаргалка: Топ-20 портов для пентестера

| Порт     | Протокол | Сервис        | Назначение                      | Возможные действия                 |
|----------|----------|---------------|----------------------------------|------------------------------------|
| **21**   | TCP      | FTP           | Передача файлов                 | Брутфорс, слив файлов              |
| **22**   | TCP      | SSH           | Удалённый доступ                | Брутфорс, туннели, reverse shell   |
| **23**   | TCP      | Telnet        | Старый удалённый доступ         | Часто без авторизации              |
| **25**   | TCP      | SMTP          | Отправка почты                  | Спам, spoof, relay атаки           |
| **53**   | UDP/TCP  | DNS           | Разрешение доменных имён        | DNS spoof, exfiltration            |
| **67/68**| UDP      | DHCP          | Автовыдача IP                   | Rogue DHCP, MITM                   |
| **80**   | TCP      | HTTP          | Веб-сервер                      | XSS, SQLi, подмена логинов         |
| **110**  | TCP      | POP3          | Получение почты                 | Перехват логинов                   |
| **123**  | UDP      | NTP           | Синхронизация времени           | DDoS Amplification                 |
| **135**  | TCP      | RPC           | Windows службы                  | Lateral movement, RCE              |
| **137-139**| TCP    | NetBIOS       | Windows шаринг                  | Сниффинг, SMB атаки                |
| **143**  | TCP      | IMAP          | Почта                           | Утечки логинов                     |
| **161/162**| UDP    | SNMP          | Мониторинг устройств            | Dump, инфо об устройстве           |
| **389**  | TCP/UDP  | LDAP          | Active Directory                | Dump учёток, обход auth            |
| **443**  | TCP      | HTTPS         | Защищённый веб-доступ           | Куки, фишинг, SSL-атаки            |
| **445**  | TCP      | SMB           | Windows доступ к файлам         | EternalBlue, SMB Relay, RCE        |
| **465/587**| TCP    | SMTP over SSL | Почта с шифрованием             | Spoof, relay                       |
| **993/995**| TCP    | IMAP/POP3 SSL | Защищённая почта                | Захват логинов                     |
| **3306** | TCP      | MySQL         | Базы данных                     | SQL-инъекции, дамп                 |
| **3389** | TCP      | RDP           | Удалённый рабочий стол Windows  | Брутфорс, захват сессии            |

---

## 🧩 Подсказки

- `21–25` — всё про подключение и передачу
- `80 / 443` — веб, `HTTP / HTTPS`
- `53` — DNS, всегда мелькает в Wireshark
- `445` — SMB, часто на Windows-целях
- `3389` — RDP, любимая цель снаружи

---

🔁 Используй вместе с `nmap`, `hydra`, `wireshark`, `metasploit` и другими инструментами.
